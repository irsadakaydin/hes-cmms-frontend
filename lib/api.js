...
async function istekAt(yol, secenekler = {}) {
  console.log("[istekAt] başlıyor:", yol, "API_URL=", API_URL);
  const token = tokenAl();
  const headers = {
  ...
  // yanıt vermiyor, DNS sorunu, vb.) sonsuza kadar beklemek yerine 15
  // saniye sonra net bir hata fırlatır.
  ...
  try {
    ...
    console.log("[istekAt] yanıt geldi:", yol, "durum=", yanit.status);
  } catch (agHatasi) {
    console.error("[istekAt] AĞ HATASI (fetch başarısız oldu):", yol, agHatasi);
    if (agHatasi.name === "AbortError") {
      throw new Error("Sunucuya bağlanılamadı (15 saniye içinde yanıt gelmedi). İnternet bağlantınızı kontrol edin.");
    }
  ...
  if (!yanit.ok) {
    if (
      yanit.status === 401 &&
      (veri.hata_kodu === "TOKEN_GECERSIZ" || veri.hata_kodu === "TOKEN_EKSIK") &&
...
async function girisYap(eposta, sifre, isletme_id) {
  const veri = await istekAt("/api/v1/auth/login", {
    method: "POST",
    body: JSON.stringify({ eposta, sifre, isletme_id: isletme_id || undefined }),
  });
